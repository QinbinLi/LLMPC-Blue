...
**Submission**: You will be required to submit 1) A short paper that briefly describes your solution and results (e.g., changes on model utility and attack success rate, efficiency); 2) Your source code and model (if any). Since defense approaches can vary significantly, we will not rely on a single metric to evaluate your solution. Instead, we will consider the following: 1) the attack success rate on the test phase data (not yet released), 2) the efficiency of the defense approach, 3) the model utility (e.g., MMLU), and 4) the novelty. Therefore, your paper may focus on the above perspectives. Experts will review your paper and provide their ratings. More information will be provided later.
